defmodule SnapFramework.UseEffect do
  def do_process(old_scene, new_scene, template_file, watch_registry, effects_registry, controller) do
    diff = diff_state(old_scene.assigns, new_scene.assigns)
    new_scene = process_effects(new_scene, diff, template_file, watch_registry, effects_registry, controller)
  ...
  def process_effects(scene, %{changed: :equal}, _, _, _, _) do
    scene
  end

  def process_effects(scene, %{changed: :map_change, added: added}, template_file, watch_registry, effects_registry, controller) do
    Enum.reduce(added, scene, fn {key, value}, acc ->
      if Enum.member?(watch_registry, key) do
        SnapFramework.Scene.compile(scene, template_file)
      else
