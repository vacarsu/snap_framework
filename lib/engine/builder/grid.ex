defmodule SnapFramework.Engine.Builder.Grid do
      ...
      Enum.reduce(children, grid, fn child, grid ->
        case child do
          [type: :component, module: module, data: data, opts: opts] ->
            ...
            translate_and_render_row_item(grid, module, data, Keyword.put_new(opts, :children, children))
    ...
    grid =
      Enum.reduce(children, grid, fn child, grid ->
        case child do
          [type: :component, module: module, data: data, opts: opts] ->
            children = if opts[:do], do: opts[:do], else: nil
            translate_and_render_col_item(grid, module, data, Keyword.put_new(opts, :children, children))
  ...
  defp translate_and_render_row_item(%{next_x: next_x, next_y: next_y, graph: graph, item_width: item_width} = grid, module, data, opts) do
    ...
    %{
      ...
      | next_x: next_x + item_width,
        next_y: next_y,
        graph:
          module.add_to_graph(graph, data, Keyword.merge(opts, translate: {next_x, next_y}))
    }
  end

  defp translate_and_render_col_item(%{next_x: next_x, next_y: next_y, graph: graph, item_height: item_height} = grid, module, data, opts) do
    %{
      grid
      | next_x: next_x,
