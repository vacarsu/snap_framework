  ...
  defmacro effect_handlers() do
    quote do
      ...
      defp add(scene, {cmp_fun, state_key, opts}) do
        graph =
        scene.assigns.graph
        |> cmp_fun.(scene.assigns.state[state_key], opts)

        assign(scene, graph: graph)
      end

      defp modify(scene, {cmp_id, {cmp_fun, state_key}}) when is_atom(state_key) do
        graph =
        scene.assigns.graph
        |> Graph.modify(cmp_id, fn g -> cmp_fun.(g, scene.assigns.state[state_key], []) end)

        assign(scene, graph: graph)
      end

      defp modify(scene, {cmp_id, {cmp_fun, {:state, nested_keys}, opts}})
      when is_list(nested_keys) and is_list(opts)
      do
        value =
          Enum.reduce(nested_keys, nil, fn key, acc ->
            acc = if is_nil(acc), do: scene.assigns.state[key], else: acc[key]
          end)
        graph =
          scene.assigns.graph
      ...
      defp modify(scene, {cmp_id, {cmp_fun, {:state, nested_keys}}}) when is_list(nested_keys)
      do
        value =
          Enum.reduce(nested_keys, nil, fn key, acc ->
            acc = if is_nil(acc), do: scene.assigns.state[key], else: acc[key]
      ...
      defp modify(scene, {cmp_id, {cmp_fun, state_key, opts}}) when is_atom(state_key) and is_list(opts) do
        graph =
        scene.assigns.graph
        |> Graph.modify(cmp_id, fn g -> cmp_fun.(g, scene.assigns.state[state_key], opts) end)
